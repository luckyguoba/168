...
(function () {
  ...
  async function playEpisode(ep) {
    ...
    els.titleBarText.textContent = epTitleText(ep);
    if (epData.remoteUrl) {
  ...
  function renderEpisodeList() {
    ...
    for (let i = 1; i <= state.totalEpisodes; i++) {
      ...
      if (state.epDisplayMode !== 'num') {
        ...
        name.textContent = (state.eps[i] && state.eps[i].title) || (state.seriesName + ' 第' + i + '集');
        d.appendChild(name);
      ...
      d.addEventListener('click', () => playEpisode(i));
  ...
  function renameEpisode(ep) {
    ...
    saveState();
    renderEpisodeList();
    if (ep === state.currentEp) els.titleBarText.textContent = epTitleText(ep);
  }
  ...
  function showControls() {
    ...
    if (!els.video.paused) {
      controlsTimer = setTimeout(() => els.controls.classList.remove('show'), 2600);
    }
  }
  ...
  function cacheElements() {
    ['playerFrame','video','posterLayer','posterImg','posterEmpty','posterEmptyText','titleBar','titleBarText',
     'centerPlay','loading','controls','btnPlay','icPlay','icPause','seek','curTime','durTime',
     'btnSpeed','speedMenu','btnRatio','ratioMenu','btnLoop','btnVol','vol','btnNext','btnFull',
     'epList','episodePanel','btnEpSet','btnEpCollapse','btnEpImport','epResize','btnEpExpand','hintBar','videoZoomWrap','posterZoomWrap','btnClearAll',
     ...
     'danmakuLayer','danmakuInput','btnDanmakuSend','btnBack','btnTitleFull'].forEach(id => { els[id] = $(id); });
  }
  ...
  async function init() {
    ...
    // 渲染选集
    renderEpisodeList();
    els.titleBarText.textContent = epTitleText(state.currentEp);
    ...
    els.btnSpeed.textContent = state.speed + 'x';
    applyRatio();
    updateLoopBtn();
  ...
  function bindEvents() {
    ...
    els.video.addEventListener('ended', () => {
      ...
      if (els.btnLoop.classList.contains('active')) { els.video.play(); }
    });
    ...
    els.videoStage.addEventListener('click', (e) => {
      if (e.target.closest('.speed-menu') || e.target.closest('.ratio-menu')) return;
      const now = Date.now();
    ...
    els.speedMenu.addEventListener('click', (e) => {
      ...
      els.video.playbackRate = state.speed;
      els.btnSpeed.textContent = state.speed + 'x';
      els.speedMenu.querySelectorAll('.sm-item').forEach(x => x.classList.toggle('active', x === item));
      els.speedMenu.hidden = true;
      saveState();
    });

    // 比例
    els.btnRatio.addEventListener('click', (e) => {
      e.stopPropagation();
      els.ratioMenu.hidden = !els.ratioMenu.hidden;
      els.speedMenu.hidden = true;
    });
    els.ratioMenu.addEventListener('click', (e) => {
      const item = e.target.closest('.sm-item');
      if (!item) return;
      state.ratio = item.dataset.r;
      applyRatio();
      els.ratioMenu.hidden = true;
      els.ratioMenu.querySelectorAll('.sm-item').forEach(x => x.classList.toggle('active', x === item));
      saveState();
    });
    ...
    // 全屏：直接切换 CSS class（不依赖 Fullscreen API，安卓 WebView 也能用）
    function applyFullscreen(isFs) {
      document.body.classList.toggle('is-fullscreen', isFs);
      // 通知安卓端切换屏幕方向（全屏横屏，非全屏竖屏）
      try { if (window.androidAPI && window.androidAPI.setFullscreen) window.androidAPI.setFullscreen(isFs); } catch (e) {}
      if (isFs) {
        // 选集面板移入视频舞台成为右侧 overlay
        if (els.episodePanel.parentNode !== els.videoStage) {
          els.videoStage.appendChild(els.episodePanel);
        }
        els.episodePanel.classList.remove('collapsed');
      } else {
        const frame = els.playerFrame;
        if (els.episodePanel.parentNode !== frame) {
          frame.appendChild(els.episodePanel);
        }
      }
    }
    ...
    // 选集面板收起
    els.btnEpCollapse.addEventListener('click', () => els.episodePanel.classList.add('collapsed'));
  ...
  function updateLoopBtn() {
    els.btnLoop.classList.toggle('active', els.video.loop);
  }
  ...
  */
  ...
  function bindV5Features() {
    // --- 选集收起/展开 ---
    if (els.btnEpExpand) els.btnEpExpand.addEventListener('click', () => {
      els.episodePanel.classList.remove('collapsed');
      els.btnEpExpand.hidden = true;
    });
    // 覆盖原收起按钮：收起后显示展开按钮
    els.btnEpCollapse.addEventListener('click', () => {
      els.episodePanel.classList.add('collapsed');
      if (els.btnEpExpand) els.btnEpExpand.hidden = false;
    });

    // --- 拖动调整选集宽度（控制栏和弹幕框跟随移动） ---
    let epResizing = false;
    if (els.epResize) {
      const doResize = (clientX) => {
        // 用选集框的定位父元素计算（全屏时父元素是 videoStage，非全屏是 playerFrame）
        const parent = els.episodePanel.offsetParent || els.playerFrame;
        const rect = parent.getBoundingClientRect();
        let w = rect.right - clientX;
        w = Math.max(100, Math.min(w, rect.width * 0.6));
        els.episodePanel.style.width = w + 'px';
        // 控制栏跟随选集框左侧移动（弹幕框是控制栏子元素，自动跟随）
        if (els.controls) els.controls.style.right = w + 'px';
      };
      els.epResize.addEventListener('mousedown', (e) => {
        epResizing = true; els.epResize.classList.add('active');
        els.episodePanel.classList.add('resizing');
        document.body.style.userSelect = 'none'; e.preventDefault();
      });
      els.epResize.addEventListener('touchstart', (e) => {
        epResizing = true; els.epResize.classList.add('active');
        els.episodePanel.classList.add('resizing');
        document.body.style.userSelect = 'none';
      }, { passive: true });
      document.addEventListener('mousemove', (e) => { if (epResizing) doResize(e.clientX); });
      document.addEventListener('touchmove', (e) => {
        if (epResizing && e.touches[0]) { e.preventDefault(); doResize(e.touches[0].clientX); }
      }, { passive: false });
      const endResize = () => {
        if (!epResizing) return;
        epResizing = false; els.epResize.classList.remove('active');
        els.episodePanel.classList.remove('resizing');
    ...
    // 同步控制栏 right 到选集框宽度（仅在用户拖动后调用，初始化完全依赖 CSS）
    function syncControlsRight() {
      if (!els.episodePanel || !els.controls) return;
      const epWidth = els.episodePanel.offsetWidth;
      // 只有选集框渲染完成（宽度>50）才设置，避免初始化时 offsetWidth=0 导致布局错乱
      if (epWidth > 50) {
        els.controls.style.right = epWidth + 'px';
      }
    }
    ...
    if (els.btnClearAll) els.btnClearAll.addEventListener('click', async () => {
      ...
      try {
        ...
        renderEpisodeList();
        els.titleBarText.textContent = epTitleText(1);
